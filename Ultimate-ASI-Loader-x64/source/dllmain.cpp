...
void LoadOriginalLibrary()
{
    ...
    else if (iequals(szSelfName, L"bink2w64.dll"))
    {
...
void LoadFile(WIN32_FIND_DATAW* fd, const std::string& fileExt) {
    ...
    if (!(fd->dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
        ...
        std::string fileExtHigher = fileExt;
        std::transform(fileExtLower.begin(), fileExtLower.end(), fileExtLower.begin(), [](unsigned char c) { return std::tolower(c); });
        std::transform(fileExtHigher.begin(), fileExtHigher.end(), fileExtHigher.begin(), [](unsigned char c) { return std::tolower(c); });

        if (fd->cFileName[pos - 4] == '.' &&
            (fd->cFileName[pos - 3] == fileExtLower[0] || fd->cFileName[pos - 3] == fileExtHigher[0]) &&
            (fd->cFileName[pos - 2] == fileExtLower[1] || fd->cFileName[pos - 2] == fileExtHigher[1]) &&
            (fd->cFileName[pos - 1] == fileExtLower[2] || fd->cFileName[pos - 1] == fileExtHigher[2])) {
...
void FindFiles(WIN32_FIND_DATAW* fd) {
    {
        WIN32_FIND_DATAW onlineFixFD{};
        HANDLE EGameSDKFile = FindFirstFileW(L"OnlineFix64.dll", &onlineFixFD);
        if (EGameSDKFile != INVALID_HANDLE_VALUE) {
            LoadFile(&onlineFixFD, "dll");
            FindClose(EGameSDKFile);
        }
    }

    {
        WIN32_FIND_DATAW sdkFD{};
        HANDLE EGameSDKFile = FindFirstFileW(L"EGameSDK.dll", &sdkFD);
        if (EGameSDKFile != INVALID_HANDLE_VALUE) {
            LoadFile(&sdkFD, "dll");
            FindClose(EGameSDKFile);
        }
